...
public class PhysicsHand : MonoBehaviour
{
    ...
    void PIDMovement()
    {
        //shows how much force we are appliying to hands in order to get to the transform of the controller
        float kp = (6f * frequency) * (6f * frequency) * 0.25f;
        float kd = 4.5f * frequency * damping;
        float g = 1 / (1 + kd * Time.fixedDeltaTime + kd * Time.fixedDeltaTime * Time.fixedDeltaTime);
        float ksg = kp * g;
    ...
    void HookesLaw()
    {
        //to propell the player into the air and allow the physics, more drag = moving slow, less drag = moving fast
